...
void print_cool_token(int tok)
{
  ...
  case (STR_CONST):
    ...
    cerr << "\"";
#ifdef CHECK_TABLES
    stringtable.lookup_string(yylval.symbol->get_string());
#endif
    break;
  case (INT_CONST):
    cerr << " = " << yylval.symbol;
#ifdef CHECK_TABLES
    inttable.lookup_string(yylval.symbol->get_string());
#endif
    break;
  case (BOOL_CONST):
    cerr << (yylval.boolean ? " = true" : " = false");
    break;
  case (TYPEID):
  case (OBJECTID):
    cerr << " = " << yylval.symbol;
#ifdef CHECK_TABLES
    idtable.lookup_string(yylval.symbol->get_string());
#endif
    break;
  case (ERROR): 
    cerr << " = ";
